...
pyol_parameters_write = pyol.ol_parameters_write
    ...
    def __init__(self, commit = True, **parameters):
        ...
        if commit:
            ...
            for key, val in self.parameters.items():
                pyol_setparameter(c_char_p(key), c_char_p(val), byref(err))
                if err.value != 0:
                    raise OpenLoopsError("Unknown parameter " + repr(key))
...
class Process:
    ...
    def __init__(self, name, loops="auto"):
        ...
        self.subprocesses = set(self.info.keys())
...
class ProcessTestData:
    ...
    def validate(self, new_process_name=None, verbose=0):
        ...
        if self.all_subprocesses:
            ...
            missing_sps = self_sp_names - process.subprocesses
            new_sps = process.subprocesses - self_sp_names
            if missing_sps:
