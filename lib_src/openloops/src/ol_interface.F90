...
module openloops
  ...
  type processinfos
    ...
    character(len=max_parameter_length) :: MAP
    character(len=max_parameter_length) :: MAPPERM
    character(len=max_parameter_length) :: APPROX
    character(len=4) :: LOOPSPEC
  ...
  function register_process(process, amptype)
    ...
    next = next + 1
    ext(next) = to_int(trim(adjustl(inp(1:index(inp, " ")))))
    ...
    register_process = register_process_id(ext(1:next), amptype)
  ...
  function register_process_id(ext_in, amptype)
    ...
    process = trim(to_string(ext_in(1))) // " " // trim(to_string(ext_in(2))) // " ->"
    do i=3,next
    ...
    if (amptype == 99) then ! write shopping list
      ...
      librarytype = 0
      do
        check = check_process(process, proc, perm, amptype, librarytype)
        if (check > 0) then ! found & registered
          ...
          if (check == -1) then  ! not found --> check collections
            check = check_process(process, proc, perm, 999, librarytype)
          end if
  ...
  function check_process(process, proc, perm, amptype, librarytype)
  ! 1: found, 0: not found, -1: abort
    ...
    InfoLoop: do
      ...
      if ( trim(proc) /= trim(process_infos(p)%PROC) &
        & .or. trim(loops_specification) /= trim(process_infos(p)%LOOPSPEC) &
        & ) then
        cycle InfoLoop
      end if
      ...
      if (amptype == 999) then
        ...
        libfilename = 'libopenloops_' // trim(process_infos(p)%LIBNAME) // '_' // &
                      & trim(process_infos(p)%LOOPSPEC) // '.' // dynlib_extension
      end if
  ...
  subroutine check_parameters(p,amptype,found)
    use ol_parameters_decl_/**/DREALKIND, only: &
      ...
      & YE, YM, YL, YU, YD, YC, YS, YB, YT, &
      & leadingcolour, coupling_QCD, coupling_EW, &
      & approximation, ckmorder, allowed_libs
    ...
    found = (process_infos(p)%EWorder(0) == coupling_EW(0) .or. coupling_EW(0) == -1)
    found = found .and. (process_infos(p)%EWorder(1) == coupling_EW(1) .or. coupling_EW(1) == -1)
    found = found .and. (process_infos(p)%QCDorder(0) == coupling_QCD(0) .or. coupling_QCD(0) == -1)
    found = found .and. (process_infos(p)%QCDorder(1) == coupling_QCD(1) .or. coupling_QCD(1) == -1)
    found = found .and. (process_infos(p)%LeadingColour == leadingcolour)
    found = found .and. (process_infos(p)%NC == nc)
    found = found .and. (process_infos(p)%NF == nf)
    found = found .and. (process_infos(p)%CKMorder == ckmorder)
    found = found .and. ((process_infos(p)%ME /= "0" .and. rME /= 0) .or. (rME == 0))
    found = found .and. ((process_infos(p)%MM /= "0" .and. rMM /= 0) .or. (rMM == 0))
    found = found .and. ((process_infos(p)%ML /= "0" .and. rML /= 0) .or. (rML == 0))
    found = found .and. ((process_infos(p)%MU /= "0" .and. rMU /= 0) .or. (rMU == 0))
    found = found .and. ((process_infos(p)%MD /= "0" .and. rMD /= 0) .or. (rMD == 0))
    found = found .and. ((process_infos(p)%MS /= "0" .and. rMS /= 0) .or. (rMS == 0))
    found = found .and. ((process_infos(p)%MC /= "0" .and. rMC /= 0) .or. (rMC == 0))
    found = found .and. ((process_infos(p)%MB /= "0" .and. rMB /= 0) .or. (rMB == 0))
    found = found .and. (rME == YE  .or. process_infos(p)%YE == 1)
    found = found .and. (rMM == YM  .or. process_infos(p)%YM == 1)
    found = found .and. (rML == YL  .or. process_infos(p)%YL == 1)
    found = found .and. (rMU == YU  .or. process_infos(p)%YU == 1)
    found = found .and. (rMD == YD  .or. process_infos(p)%YD == 1)
    found = found .and. (rMS == YS  .or. process_infos(p)%YS == 1)
    found = found .and. (rMC == YC  .or. process_infos(p)%YC == 1)
    found = found .and. (rMB == YB  .or. process_infos(p)%YB == 1)
    found = found .and. (rMT == YT  .or. process_infos(p)%YT == 1)
    found = found .and. (amptype == 1 .or. amptype > 10 .or. process_infos(p)%CC /= "0")
    found = found .and. (trim(process_infos(p)%APPROX) == trim(approximation) .or. len_trim(allowed_libs) /= 0)
  ...
  subroutine readAllInfoFiles(ierr, load_channel_lib)
    ...
    InfoListLoop: do
      ...
      InfoFileLoop: do
        ...
        if (info_file_suffix == 'rinfo') then
          ...
          infos%LOOPSPEC = infofilename(index(infofilename,'_',.true.)+1:index(infofilename,'.info')-1)
        end if
    ...
    subroutine readAllInfos(lineinfo, infos)
      ...
      call readInfoInt(lineinfo, 'YE', infos%YE)
      call readInfoInt(lineinfo, 'YM', infos%YM)
      call readInfoInt(lineinfo, 'YL', infos%YL)
      call readInfoInt(lineinfo, 'YU', infos%YU)
      call readInfoInt(lineinfo, 'YD', infos%YD)
      call readInfoInt(lineinfo, 'YS', infos%YS)
      call readInfoInt(lineinfo, 'YC', infos%YC)
      call readInfoInt(lineinfo, 'YB', infos%YB)
      call readInfoInt(lineinfo, 'YT', infos%YT)
...
!    end do
  ...
  function write_shopping_list(ext, proc)
    ...
    ! process name and id
    output = "(* " //trim(proc)// " *) SubProcess[" // trim(to_string(id)) // "] = {FeynArtsProcess -> "
    ! inital state
  ...
  subroutine tree_colbasis_dim(id, ncolb)
    ! number of tree colour basis elements
    implicit none
    integer, intent(in) :: id
    integer, intent(out) :: ncolb
  ...
  subroutine tree_colbasis_dim_c(id, ncolb) bind(c,name="ol_tree_colbasis_dim")
    implicit none
    integer(c_int), value :: id
    integer(c_int), intent(out) :: ncolb
    integer :: f_ncolb
    call tree_colbasis_dim(int(id), f_ncolb)
    ncolb = f_ncolb
  ...
  pure function get_ncolext(id)
    ...
    call process_handles(id)%tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel)
    get_ncolext = count(extcols /= 0)
  end function get_ncolext
  ...
  subroutine tree_colbasis_c(id, basis, needed) bind(c,name="ol_tree_colbasis")
    implicit none
    integer(c_int), value :: id
    integer(c_int), intent(out) :: basis(get_ncolext(id)/2+get_ncolext(id)-1,get_tree_colbasis_dim(id))
    integer(c_int), intent(out) :: needed(get_tree_colbasis_dim(id),get_tree_colbasis_dim(id))
    integer :: f_basis(get_ncolext(id)/2+get_ncolext(id)-1,get_tree_colbasis_dim(id))
  ...
  subroutine evaluate_cc_c(id, pp, tree, cc, ewcc) bind(c,name="ol_evaluate_cc")
    ...
    integer(c_int), value :: id
    real(c_double), intent(in) :: pp(5*process_handles(id)%n_particles)
    real(c_double), intent(out) :: tree, cc(rval_size(process_handles(id)%n_particles,2)), ewcc
    integer :: f_id
    real(DREALKIND) :: f_pp(0:4,process_handles(id)%n_particles)
    real(DREALKIND) :: f_tree, f_cc(rval_size(process_handles(id)%n_particles,2)), f_ewcc
    f_id = id
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccmatrix_c(id, pp, tree, ccij, ewcc) bind(c,name="ol_evaluate_ccmatrix")
    ...
    integer(c_int), value :: id
    real(c_double), intent(in) :: pp(5*process_handles(id)%n_particles)
    real(c_double), intent(out) :: tree, ccij(process_handles(id)%n_particles,process_handles(id)%n_particles), ewcc
    integer :: f_id
    real(DREALKIND) :: f_pp(0:4,process_handles(id)%n_particles)
    real(DREALKIND) :: f_tree, f_ccij(process_handles(id)%n_particles,process_handles(id)%n_particles), f_ewcc
    f_id = id
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
