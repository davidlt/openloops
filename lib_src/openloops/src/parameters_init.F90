...
subroutine parameters_init()
  ...
  use ol_parameters_decl_/**/DREALKIND, only: &
    ...
    & rMX_dp => rMX, wMX_dp => wMX, rMY_dp => rMY, wMY_dp => wMY, YE_dp => YE, YM_dp => YM, YL_dp => YL, &
    & YU_dp => YU, YD_dp => YD, YS_dp => YS, YC_dp => YC, YB_dp => YB, YT_dp => YT
  implicit none
